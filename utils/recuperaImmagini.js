        ...
        parole.map(async (parola) => {
            try {
                const response = await fetch(
                    `https://api.unsplash.com/search/photos?query=${encodeURIComponent(parola)}&per_page=1&content_filter=high&client_id=${process.env.IMAGE_API_KEY}`
                );
                if (!response.ok) {
                ...
                return `${data.results[0].urls.raw}&w=400&h=400&fit=crop`;
